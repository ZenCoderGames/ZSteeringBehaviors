...
func get_nearby_characters(pos:Vector2, radius:float, groupId:int=-1, ignoreGroupId:int=-1)->Array[ZAI_Character]:
	...
	for character in characters:
		if character.global_position.distance_to(pos) <= radius:
			if groupId == -1 or character.groupId == groupId:
				if ignoreGroupId == -1 or character.groupId != ignoreGroupId:
