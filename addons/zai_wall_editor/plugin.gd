...
func _handles(object: Object) -> bool:
	return object is ZAI_Wall

func _edit(object: Object) -> void:
	if object is ZAI_Wall:
		edited_wall = object as ZAI_Wall
	else:
		edited_wall = null
...
func _forward_canvas_draw_over_viewport(viewport_control: Control) -> void:
	if edited_wall == null or edited_wall.points.size() == 0:
		return

	var canvas_transform:Transform2D = edited_wall.get_viewport_transform() * edited_wall.get_global_transform()

	# Draw handles for each point
	for i in range(edited_wall.points.size()):
		var point_pos:Vector2 = canvas_transform * edited_wall.points[i]
...
func _forward_canvas_gui_input(event: InputEvent) -> bool:
	...
	if event is InputEventMouseButton:
		...
		if dragging_point != -1:
			...
			# Update the point
			var new_points:PackedVector2Array = edited_wall.points.duplicate()
			new_points[dragging_point] = global_pos
			edited_wall.points = new_points
